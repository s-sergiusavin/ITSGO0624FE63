...
const singleUserData = fetch(singleUserUrl).then(response => response.json());
...
const createuser = new Promise((resolve, reject) => {
    ...
    }) // pana in punctul asta se face request-ul
        .then(response => {
            console.log(response);
            if (response.status !== 201) {
                throw 'Ai o eroare la crearea user-ului'
            }
            return response.json();
        })
        .then(data => {
...
const registerUser = new Promise((resolve, reject) => {
    fetch(registerUrl, registerUrlConfig) // pana in punctul asta, se face request-ul
        .then(response => {
            if (response.status !== 200) {
...
const deleteUser = new Promise((resolve, reject) => {
    ...
    .then(response => {
        if (response.status !== 204) {
            throw 'Ai o eroare la stergerea user-ului';
        }
    })
})